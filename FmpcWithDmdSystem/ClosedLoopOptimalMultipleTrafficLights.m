...
initial_point           = 10*ones(3*tf-3,1);
initial_point(1:tf-1,1) = zeros(tf-1,1);
...
for i = 1:NumberTrafficLights
    ...
    for k = 1:tf-1
        ...
        initial_point(2*tf-1:3*tf-3,1) = design_sol(2*tf-1:3*tf-3,1);
...
MPGOptimal = FuelEfficiencyMPG(sHostTotal, vHostTotal, uHostTotal, Ts);
