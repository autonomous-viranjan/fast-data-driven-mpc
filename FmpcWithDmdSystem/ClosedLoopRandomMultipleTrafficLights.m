...
for i = 1:NumberTrafficLights
    ...
    for k = 1:tf-1
        ...
        [sHost, vHost, uHost, MaxMPG, CPU, SuccesfulBranches] = ...
            RandomMethodFunction(time((i-1)*(tf-1) + k:(i-1)*(tf-1) + tf,1),...
    ...
    MPG(i,1) = FuelEfficiencyMPG(sHostTotal, vHostTotal, uHostTotal, Ts);
end

TotalMPG = FuelEfficiencyMPG(sHostTotal, vHostTotal, uHostTotal, Ts);
CPUIteration = CPUTotal/(NumberTrafficLights*tf);
