...
c = zeros(tf,1);
for i = 1:tf-1
    %c(i,1) = s(i+1,1) - x_lead(i+1,1) + dmin + alpha*v(i+1,1);    % constant sigma
    c(i,1) = s(i+1,1) - x_lead(i+1,1) + dmin(i) + alpha*v(i+1,1); 
    %c(i,1) = s(i+1,1) - x_lead(i+1,1) + dmin(i);    % time-varying
end
c(tf,1) = df - s(tconstraint);%<=0
